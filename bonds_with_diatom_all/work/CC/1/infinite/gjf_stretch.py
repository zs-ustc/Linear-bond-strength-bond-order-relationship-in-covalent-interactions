...
def read_gjf(filename):
    ...
    lines[3]="#p opt UM062X/6-311+G** nosymm int=superfine SCF=NoVarAcc SCF=tight\n"
    # 提取文件头（前8行）
    header = lines[:8]
    ...
    for line in lines[8:]:
        ...
        if len(parts) == 5:  # 假定原子坐标部分为5列，原子符号+坐标
            atom = parts[0]
...
def main():
    input_filename = '../force/input/16000.gjf'
    ...
    while True:
        ...
        if abs(first_coord[0]) >= 10.5:
            break
            
        # 生成新的文件名
        output_filename = f"input/{(iteration+1)*100:05d}.gjf"
